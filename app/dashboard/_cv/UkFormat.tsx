...
/**
 * UkFormat.tsx — full standalone UK CV editor
 * -------------------------------------------------------
 * - Retains all logic from CvTab.tsx (UK portion)
 * - Uses shared CvUtils for helper functions
 * - Includes prefill, Vincere API integration, upload, and preview
 * - Identical UI / layout / styling
 */

export default function UkFormat(): JSX.Element {
  // ======== UI state ========
  ...
  async function uploadPdf(blob: Blob) {
    try {
      ...
      formData.append('file', blob, `${candidateName || 'CV'}_UK.pdf`)
      ...
      alert('CV uploaded successfully to Vincere!')
    } catch (err: any) {
      setError(err.message)
    }
  }
  ...
  async function exportToPdf() {
    ...
    try {
      ...
      const opt = {
        margin: 10,
        filename: `${candidateName || 'CV'}_UK.pdf`,
  ...
  return (
    <div className="card p-4 space-y-4">
      ...
      <div ref={previewRef} className="border-t mt-6 pt-4">
        <h2 className="font-semibold text-lg mb-2">Preview (UK Format)</h2>
        <div className="text-sm leading-relaxed space-y-2">
          ...
          <p>{form.profile}</p>
          <p>
            <strong>Key Skills:</strong> {form.keySkills}
          </p>
        </div>
      </div>
    </div>
