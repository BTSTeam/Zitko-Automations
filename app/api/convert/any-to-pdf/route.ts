...
function sniffInputFormat(name: string, mime?: string | null): string | undefined {
  ...
  return undefined // let CloudConvert infer if needed
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Fast-path: already a PDF → return as-is
    ...
    // 1) Create CloudConvert job: import → convert(any→pdf, engine=office) → export/url
    ...
    // 2) Upload the original file to CloudConvert’s signed URL
